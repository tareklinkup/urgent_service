...
@section("title", "Hospital password change")
        ...
        <div class="card">
            <form id="updateImage">
                <div class="form-group text-center">
                    <img class="img" src="{{asset(Auth::guard('hospital')->user()->image != 0 ? Auth::guard('hospital')->user()->image : '/noimage.jpg')}}" width="100">
                </div>
...
<script>
    $(document).ready(() => {
        $("#updatePassword").on("submit", (event) => {
            ...
            $.ajax({
                url: "{{route('hospital.password.update')}}",
                data: formdata,
                method: "POST",
                dataType: "JSON",
                contentType: false,
        ...
        $("#updateImage").on("submit", (event) => {
            ...
            var formdata = new FormData(event.target)
            $.ajax({
                url: "{{route('hospital.image.update')}}",
                data: formdata,
                method: "POST",
                dataType: "JSON",
                contentType: false,
