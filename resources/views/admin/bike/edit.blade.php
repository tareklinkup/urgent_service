                ...
                <div class="card-body p-3">
                    <form id="updateBike">
                        ...
                        <div class="row">
                            ...
                            {{-- <div class="col-md-4">
                                @php
                                    $car = \App\Models\CategoryWisePrivatecar::where('privatecar_id', $data->id)
                                        ->pluck('cartype_id')
                                        ->toArray();
                                @endphp
    ...
    <script>
        ...
        $(document).ready(() => {
            $("#updateBike").on("submit", (event) => {
                ...
                $.ajax({
                    url: "{{ route('admin.bike.update') }}",
                    data: formdata,
                    method: "POST",
                    contentType: false,
                    processData: false,
                    beforeSend: () => {
                        $("#updateBike").find(".error").text("");
                    },
                    success: (response) => {
                        if (response.error) {
                            $.each(response.error, (index, value) => {
                                $("#updateBike").find(".error-" + index).text(
                                    value);
                            })
                        } else {
                            $("#updateBike").trigger('reset')
