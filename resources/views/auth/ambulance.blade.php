...
<body>
    ...
    <div class="wrapper">
        ...
        <form class="p-3 mt-3" id="ambulance">
    ...
    <script>
        $(document).ready(() => {
            ...
            $("#ambulance").on("submit", (event) => {
                event.preventDefault()
                var formdata = new FormData(event.target)
                $.ajax({
                    url: "{{route('ambulance.login')}}",
                    data: formdata,
                    method: "POST",
                    dataType: "JSON",
                    contentType: false,
                    processData: false,
                    beforeSend: () => {
                        $("#ambulance").find(".error").text("");
                    },
                    success: (response) => {
                        if (response.error) {
                            $.each(response.error, (index, value) => {
                                $("#ambulance").find(".error-" + index).text(value);
                            })
                        } else if (response.errors) {
                            alert(response.errors)
                        } else {
                            $("#ambulance").trigger('reset')
                            window.location.href = "{{route('ambulance.dashboard')}}"
                        }
                    }
                })
            })
