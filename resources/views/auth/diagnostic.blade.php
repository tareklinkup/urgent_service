...
<body>
    ...
    <div class="wrapper">
        ...
        <form class="p-3 mt-3" id="diagnostic">
    ...
    <script>
        $(document).ready(() => {
            ...
            $("#diagnostic").on("submit", (event) => {
                event.preventDefault()
                var formdata = new FormData(event.target)
                $.ajax({
                    url: "{{route('diagnostic.login')}}",
                    data: formdata,
                    method: "POST",
                    dataType: "JSON",
                    contentType: false,
                    processData: false,
                    beforeSend: () => {
                        $("#diagnostic").find(".error").text("");
                    },
                    success: (response) => {
                        if (response.error) {
                            $.each(response.error, (index, value) => {
                                $("#diagnostic").find(".error-" + index).text(value);
                            })
                        } else if (response.errors) {
                            alert(response.errors)
                        } else {
                            $("#diagnostic").trigger('reset')
                            window.location.href = "{{route('diagnostic.dashboard')}}"
                        }
                    }
                })
            })
