...
<body>
    <div class="wrapper">
        ...
        <form class="p-3 mt-3" id="privatecar">
            ...
            <button type="submit" class="btn mt-3">Privatecar</button>
        </form>
    ...
    <script>
        $(document).ready(() => {
            ...
            $("#privatecar").on("submit", (event) => {
                event.preventDefault()
                var formdata = new FormData(event.target)
                $.ajax({
                    url: "{{route('privatecar.login')}}",
                    data: formdata,
                    method: "POST",
                    dataType: "JSON",
                    contentType: false,
                    processData: false,
                    beforeSend: () => {
                        $("#privatecar").find(".error").text("");
                    },
                    success: (response) => {
                        if (response.error) {
                            $.each(response.error, (index, value) => {
                                $("#privatecar").find(".error-" + index).text(value);
                            })
                        } else if (response.errors) {
                            alert(response.errors)
                        } else {
                            $("#privatecar").trigger('reset')
                            window.location.href = "{{route('privatecar.dashboard')}}"
                        }
                    }
                })
            })
