                                ...
                                {{-- <div class="col-md-3 col-10">
                                    <div class="form-group">
                                        ...
                                        <select class="rounded-pill" name="privatecar_type" id="privatecar_type">
                                            <option label="Select Car Type"></option>
            ...
            <div class="row m-md-0" style="border: 1px solid #e5e5e5;">
                <div class="col-md-3 p-md-0 order-md-0 order-last">
                    <div class="card border-0"
                        style="border-radius: 0;height:100%;border-right: 1px solid #e3e3e3 !important;">
                        ...
                        <div class="card-body" style="padding-top: 3px;">
                            <a title="All" href="{{ route('privatecar.details') }}"
                                class="privatecar_category {{ $type_id != null ? '' : 'text-danger' }}">All</a>
                            @foreach ($categories as $item)
                                <a title="{{ $item->name }}"
                                    href="{{ route('privatecar.details', ['type', 'id' => $item->id]) }}"
                                    class="privatecar_category {{ $type_id == $item->id ? 'text-danger' : '' }}">{{ $item->name }}
                                    <span class="text-danger"
                                        style="font-weight:700;">({{ $item->typewiseprivatecar->count() }})</span></a>
                            @endforeach
                            ...
                            <div style="height: 400px;overflow-y: scroll;">
                                @foreach ($cities as $item)
                                    <a title="{{ $item->name }}"
                                        href="{{ route('privatecar.details', ['city', 'id' => $item->id]) }}"
                                        class="privatecar_category {{ $item->id == $city_id ? 'text-danger' : '' }}">{{ $item->name }}
                                        <span class="text-danger"
                                            style="font-weight:700;">({{ $item->privatecar->count() }})</span></a>
                                @endforeach
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-9 doctor_details">
                    <h5 class="m-0 totalDoctorcount" style="text-align: right;font-family: auto;font-style: italic;">Total:
                        <span>{{ $data['privatecar']->count() }}</span>
                    </h5>
                        ...
                        @foreach ($data['privatecar'] as $item)
                            @if ($item->privatecar != null)
                                <div class="col-md-6 mb-3">
                                    <a href="{{ route('singlepageprivatecar', $item->privatecar->id) }}" target="_blank"
                                        class="text-decoration-none text-secondary" title="{{ $item->privatecar->name }}">
                                        <div class="card"
                                            style="border-radius: 0;border: 0;font-family: auto;box-shadow: 0px 0px 8px 0px #bfbfbfbf;height:130px;">
                                            <div class="card-body d-flex" style="padding: 5px;gap: 8px;">
                                                <div class="image"
                                                    style="border: 1px dotted #ababab;height: 110px;margin-top: 4px;">
                                                    <img src="{{ asset($item->privatecar->image != '0' ? $item->privatecar->image : '/frontend/img/privatecar.png') }}"
                                                        width="100" height="100%">
                                                </div>
                                                <div class="info" style="padding-right:5px;">
                                                    <h6>{{ $item->privatecar->name }}</h6>
                                                    <p style="color:#c99913;">{{ $item->cartype->name }},
                                                        {{ $item->privatecar->city->name }}</p>
                                                    <p style="border-top: 2px dashed #dddddd85;text-align:justify;"><i
                                                            class="fa fa-map-marker"></i>
                                                        {{ mb_strimwidth($item->privatecar->address, 0, 100, '...') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </a>
            ...
            $.ajax({
                ...
                success: res => {
                    if (res.status == true && res.message.length == 0) {
                        ...
                        $(".privatecarbody").html(
                            `<div class="bg-dark text-white text-center">${res.message}</div>`)
                    }
                },
                complete: () => {
